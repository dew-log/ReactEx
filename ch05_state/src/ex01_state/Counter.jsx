...
; (마치 갈고리를 걸은 것처럼 변경사항을 모니터링)
 ...
 1) 지역변수 number를 쓸 때의 문제점
    ; Counter 함수 내에 선언하게 되면
      Counter가 다시 그려질 때(Rendering)
      같이 초기화 된다는 단점이 있다.

 2) 전역변수 number를 쓸 때의 문제점
    ; Counter 함수 외부의 변수와 함수가 연결 지어지므로
...
3) 위의 2가지에 대한 대안으로 state를 만들었다.
    ...
    ReRendring과 관계없이 값이 유지된다
*/

function Counter(){
    // state 변수, state 함수  = number의 초기값
...
return (
    <div>
      ...
      <button onClick={() => {
          ...
          /* setNumber의 역할
          1) number = number-1
          2) React엔진에 Counter의 ReRendring 이벤트를 전달
          */
        }}> ++ </button>
    </div>
  );
